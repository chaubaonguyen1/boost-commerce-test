...
export const SuggestionFilter: FunctionComponent<ISuggestionFilter> = (
  props
) => {
  const {
    itemsWithImg,
  ...
  const ProductComponent = () => {
    return (
      <>
        ...
        {renderItemsWImg.length === 0 && <div className="no-result">No result</div>}
        {(limitItemsWithImg
          ? renderItemsWImg.slice(0, limitItemsWithImg)
          : renderItemsWImg
        ).map((item, index) => {
          return (
            <a href={item.product.url}>
              <CardItems {...item.product} key={index} />{" "}
            </a>
          );
        })}
      </>
  ...
  const SuggestionComponent = () => {
    ...
    for (let index = 0; index < renderSuggestion.length; index++) {
      ...
      if (
        !(
          totalSuggestion.filter(
            (currentItem) => currentItem.term === item.suggestionWord.term
          ).length > 0
        )
      ) {
    ...
    return (
      <>
        ...
        {totalSuggestion.length === 0 && <div className="no-result">No result</div>}
  ...
  const CollectionComponent = () => {
    ...
    for (let index = 0; index < renderCollection.length; index++) {
      ...
      if (
        !(
          totalCollection.filter(
            (currentItem) => currentItem.title === item.collection.title
          ).length > 0
        )
      ) {
    ...
    return (
      <>
        ...
        {totalCollection.length === 0 && <div className="no-result">No result</div>}
        {totalCollection.map((item, index) => {
          return (
            <a className="link-item" href={item.url} key={index}>
              {item.title}
            </a>
          );
        })}
  ...
  const handleChange = (event) => {
    ...
    if (
      event &&
      event.target.value !== "" &&
      event.target.value.length >= minSearchLetter
    ) {
            ...
            {({ TransitionProps }) => (
              <Fade {...TransitionProps} timeout={350}>
                <Paper>
                  ...
                  {renderItemsWImg.length > 0 && (
                    <div className="">
                      View more {renderItemsWImg.length - limitItemsWithImg}
                    </div>
                  )}
                </Paper>
              </Fade>
